...
seqGLMM_GxG_spa <- function(formula, data, gds_grm, gds_assoc, snp_pair,
    ...
    tau.init=c(0,0), approx_init_tau=TRUE, approx_use_tau=FALSE,
    traceCVcutoff=0.0025, ratioCVcutoff=0.001, geno.sparse=TRUE,
    ...
    verbose.detail=TRUE)
{
    ...
    stopifnot(is.logical(approx_init_tau), length(approx_init_tau)==1L)
    stopifnot(is.logical(approx_use_tau), length(approx_use_tau)==1L)
    if (isTRUE(approx_use_tau) && !isTRUE(approx_init_tau))
        stop("'approx_init_tau' should be TRUE if 'approx_use_tau=TRUE'.")
    ...
    approx_use_tau <- isTRUE(approx_use_tau)
    ...
    if (isTRUE(approx_init_tau))
    {
        ...
        if (!approx_use_tau)
        {
            if (tau.init[1L] <= 0) tau.init[1L] <- fixtau[1L]
            if (tau.init[2L] <= 0) tau.init[2L] <- fixtau[2L]
        }
        if (verbose) cat("    ")
    } else {
        # no initial guess
    ...
    if (verbose)
    {
        if (approx_use_tau) cat("Use") else cat("Initial")
        .cat(" tau for the interaction: (", tau.init[1L], ", ", tau.init[2L], ")")
    }


    ####  Enumerate each SNP pair  ####

    param$no_iteration <- approx_use_tau
